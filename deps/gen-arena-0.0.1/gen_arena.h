...
public:
    static GenArenaTypedRef from_uint64(uint64_t data) {
        static_assert(sizeof(GenArenaTypedRef) == 8, "GenArenaRef is larger than 8 bytes!");
        GenArenaTypedRef ref;
        memcpy(&ref, &data, sizeof(GenArenaTypedRef));
        return ref;
    }
    static GenArenaTypedRef from_userpointer(void* data) {
        static_assert(sizeof(GenArenaTypedRef) == 8, "GenArenaRef is larger than 8 bytes!");
        GenArenaTypedRef ref;
        memcpy(&ref, &data, sizeof(GenArenaTypedRef));
        return ref;
    ...
    uint64_t to_uint64() const {
        uint64_t res;
        memcpy(&res, this, sizeof(GenArenaTypedRef));
        return res;
    }
    ...
    bool operator==(const GenArenaTypedRef& other) const {
        return memcmp(this, &other, sizeof(GenArenaTypedRef)) == 0;
    }
    ...
    bool operator<(const GenArenaTypedRef& other) const {
        return memcpy(this, &other, sizeof(GenArenaTypedRef)) < 0;
    }
...
private:
    static constexpr uint32_t get_safe_type_id() {
        constexpr uint32_t tid = gen_arena_type_id<T>();
        constexpr uint32_t tid_max = (1 << Config::TypeIdBits) - 1;
        static_assert (tid == UINT32_MAX || tid <= tid_max, "Type id is invalid!");
        if constexpr (tid == UINT32_MAX) {
            // type id is unspecified, should be truncated in the right way
            return tid_max - 1;
        }
        else if constexpr (tid <= tid_max) {
            // type id is OK
            return tid;
        }
        else {
